...
const EventForm: React.FC<EventFormProps> = ({ onSubmit, onCancel, isSubmitting = false }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title) {
      alert(t('eventForm.titleRequired'));
      return;
    }
    ...
    if (finalEndDate.getTime() <= finalStartDate.getTime()) {
      alert(t('eventForm.endAfterStart'));
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div
          ...
          className={`p-3 rounded-lg cursor-pointer border-2 transition-colors ${activeDateSelection === 'start' ? 'tenant-border-200 tenant-bg-50' : 'border-gray-200 bg-white hover:bg-gray-50'
            } ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <p className="text-xs font-bold text-gray-500 uppercase">{t('eventForm.startDate')}</p>
          <p className="font-semibold text-gray-800">{formatDateForDisplay(startDate)}</p>
        </div>
        <div
          onClick={() => !isSubmitting && setActiveDateSelection('end')}
          className={`p-3 rounded-lg cursor-pointer border-2 transition-colors ${activeDateSelection === 'end' ? 'tenant-border-200 tenant-bg-50' : 'border-gray-200 bg-white hover:bg-gray-50'
            } ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <p className="text-xs font-bold text-gray-500 uppercase">{t('eventForm.endDate')}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <label htmlFor="startTime" className="block text-sm font-medium text-gray-700 mb-1">{t('eventForm.startTime')}</label>
          <input
            type="time"
            id="startTime"
            name="startTime"
        ...
        <div>
          <label htmlFor="endTime" className="block text-sm font-medium text-gray-700 mb-1">{t('eventForm.endTime')}</label>
          <input
            type="time"
            id="endTime"
            name="endTime"
      ...
      <ToggleSwitch
        label={t('eventForm.isOnlineEvent')}
        enabled={isOnline}
      ...
      <div className="flex justify-end items-center space-x-4 pt-4 border-t border-gray-200">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? t('eventForm.creatingEvent') : t('eventForm.createEvent')}
        </Button>
      </div>
    </form>
  );
