...
const SmallGroupCard: React.FC<SmallGroupCardProps> = ({ group, currentUser, onUpdate, onOpen }) => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        if (created && created.status === 'APPROVED') {
          ...
          alert(t('smallGroups.requestPendingAlert'));
        }
        return;
      }
      if (res.status === 409) {
        // Already a member
        onUpdate();
        alert(t('smallGroups.alreadyMember'));
        return;
      }
      if (res.status === 403) {
        alert(t('smallGroups.notAllowedJoin'));
        return;
      }
      ...
      console.error(err);
      alert(t('smallGroups.joinFailed'));
    }
  };
  ...
  const handleLeave = async () => {
    try {
      ...
      console.error(err);
      alert(t('smallGroups.leaveFailed'));
    }
  };
  ...
  const ActionButton = () => {
    if (isUserMember) {
      return (
        <Button variant="secondary" onClick={handleLeave}>{t('smallGroups.leaveGroup')}</Button>
      );
    }
    if (isPendingMember) {
      return <Button variant="secondary" disabled>{t('smallGroups.requestPending')}</Button>
    }
    return <Button onClick={handleJoin}>{t('smallGroups.joinGroup')}</Button>
  }
    ...
    <Card className="!p-0 flex flex-col h-full">
      <div className="p-6 flex-grow">
        ...
        <div className="mt-2">
          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${group.isHidden ? 'bg-yellow-100 text-yellow-800' : (group.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')}`}>
            {group.isHidden ? t('smallGroups.hidden') : (group.isActive ? t('smallGroups.active') : t('smallGroups.inactive'))}
          </span>
        </div>
        ...
        <div className="mt-6 border-t border-gray-200 pt-4">
          <div className="flex items-center space-x-3">
            <UserLink userId={group.leader?.id} className="flex items-center space-x-3">
              <Avatar src={group.leader?.profile?.avatarUrl || '/placeholder-avatar.svg'} name={group.leader?.profile?.displayName || group.leader?.email || t('roles.leader')} size="md" className="w-10 h-10" />
              <div>
                <p className="text-xs text-gray-500">{t('roles.leader')}</p>
                <p className="text-sm font-medium text-gray-800">{group.leader?.profile?.displayName || group.leader?.email || t('roles.leader')}</p>
              </div>
            </UserLink>
          </div>
        </div>
        <div className="mt-4">
          <h4 className="text-xs font-bold uppercase text-gray-500">{t('smallGroups.members', { count: members.length })}</h4>
      ...
      <div className="bg-gray-50 px-6 py-4 text-right">
        <div className="flex items-center justify-end space-x-2">
          <Button variant="ghost" onClick={() => onOpen && onOpen()}>{t('common.view')}</Button>
          <ActionButton />
        </div>
      </div>
    </Card>
