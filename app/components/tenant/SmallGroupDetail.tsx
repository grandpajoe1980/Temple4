...
export default function SmallGroupDetail({ tenantId, groupId, currentUser, onClose, onRefresh, isAdmin }: SmallGroupDetailProps) {
  ...
  if (loading) return <div className="p-6 bg-white rounded-lg shadow-sm">Loading...</div>;
    ...
    <>
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              {(() => {
                ...
                return (
                  <div>
                    <button onClick={() => setShowResourceModal(true)} className="px-3 py-1 bg-indigo-600 text-white rounded">Add Resource</button>
                  </div>
                );
              })()}
            </div>
            ...
            <div className="mt-3">
              ...
              {!resLoading && resources.length === 0 && <div className="text-sm text-gray-500">No resources yet.</div>}
        ...
        {tab === 'announcements' && (
          <div>
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-gray-700">Announcements</h4>
              {(() => {
                const isLeader = group.leader?.id === currentUser?.id || (group.members || []).some((mm: any) => mm.user?.id === currentUser?.id && (mm.role === 'LEADER' || mm.role === 'CO_LEADER'));
                const canManage = isLeader || !!isAdmin;
                if (!canManage) return null;
                return (
                  <div>
                    <button onClick={() => { setEditingAnnouncementId(null); setAnnouncementForm({ title: '', body: '' }); setShowAnnModal(true); }} className="px-3 py-1 bg-indigo-600 text-white rounded">New Announcement</button>
                  </div>
                );
              })()}
            </div>
            ...
            <div className="mt-3">
              ...
              {!annLoading && announcements.length === 0 && <div className="text-sm text-gray-500">No announcements yet.</div>}
              <ul className="mt-2 space-y-3">
                {announcements.map(a => (
        ...
        {tab === 'settings' && (
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">Only leaders and admins will see edit controls here.</p>
            {(() => {
              const isLeader = group.leader?.id === currentUser?.id || (group.members || []).some((mm: any) => mm.user?.id === currentUser?.id && (mm.role === 'LEADER' || mm.role === 'CO_LEADER'));
