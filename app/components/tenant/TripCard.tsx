...
export default function TripCard({ trip, onOpen }: TripCardProps) {
  ...
  const formatDateRange = (start?: Date | string | null, end?: Date | string | null) => {
    ...
    if (startDate && endDate) {
      return `${startDate.toLocaleDateString(localeCode)} → ${endDate.toLocaleDateString(localeCode)}`;
    }
    if (startDate) return startDate.toLocaleDateString(localeCode);
    return t('trips.dateTBA');
  };
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">{trip.summary || trip.description || t('trips.defaultDescription')}</p>
          <div className="mt-2 text-xs text-gray-500">{formatDateRange(trip.startDate, trip.endDate)}</div>
          {trip.costCents ? (
            <div className="mt-1 text-sm font-medium text-gray-800">
              {t('trips.cost')}: {(trip.costCents / 100).toLocaleString(localeCode, { style: 'currency', currency: trip.currency || 'USD' })}
            </div>
          ) : null}
        </div>
        {capacity > 0 ? (
          <div className="text-right">
            <div className="text-xs font-semibold text-gray-700">
              {t('trips.spots', { filled: approvedMembers.length, total: capacity })}
            </div>
