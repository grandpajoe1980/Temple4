...
const ResourceForm: React.FC<ResourceFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !fileUrl) {
      alert(t('forms.resource.requiredFields'));
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Input
        label={t('forms.resource.fileUrl')}
        id="fileUrl"
        name="fileUrl"
        type="url"
        value={fileUrl}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">{t('forms.resource.visibility')}</label>
        <div className="flex space-x-4">
          <label className="flex items-center space-x-2">
            ...
            <span>{t('forms.resource.membersOnly')}</span>
          </label>
          <label className="flex items-center space-x-2">
