...
const DonationsTab: React.FC<DonationsTabProps> = ({ tenant, onUpdate, onSave }) => {
  ...
  const handleFundSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await loadFunds();
      await loadAuditLogs();
      alert(editingFundId ? t('settings.donations.fundUpdated') : t('settings.donations.fundCreated'));
    } catch (error: any) {
      alert(error.message || t('settings.donations.saveFundFailed'));
    }
  };
  ...
  const handleArchiveFund = async (fundId: string) => {
    ...
    try {
      ...
      alert(error.message || t('settings.donations.archiveFailed'));
    }
  };
  ...
  const handleImport = async () => {
    ...
    await loadAuditLogs();
    setCsvText('');
    alert(t('settings.donations.importComplete'));
  };
  ...
  const exportDonationsCsv = async () => {
    try {
      ...
      alert(error.message || t('settings.donations.exportFailed'));
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.donations.title')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('settings.donations.description')}</p>
      </div>

      <div className="space-y-6">
        <div>
          <label htmlFor="mode" className="block text-sm font-medium text-gray-700 mb-1">{t('settings.donations.donationMode')}</label>
          <select
            id="mode"
            name="mode"
        ...
        {settings.mode === 'EXTERNAL' && (
          <Input
            label={t('settings.donations.externalUrl')}
            id="externalUrl"
            name="externalUrl"
            type="url"
      ...
      <div className="border-t border-gray-200 pt-8">
        <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.donations.donationForm')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('settings.donations.donationFormDesc')}</p>
      </div>
      <div className="space-y-6">
        <Input
          label={t('settings.donations.currency')}
          id="currency"
          name="currency"
        ...
        />
        <Input
          label={t('settings.donations.suggestedAmounts')}
          id="suggestedAmounts"
        ...
        />
        <ToggleSwitch
          label={t('settings.donations.allowCustomAmounts')}
          description={t('settings.donations.allowCustomAmountsDesc')}
      ...
      <div className="border-t border-gray-200 pt-8">
        <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.donations.waysToGive')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('settings.donations.waysToGiveDesc')}</p>
      </div>
      ...
      <div className="border-t border-gray-200 pt-8">
        <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.donations.leaderboard')}</h3>
      </div>
      <div className="space-y-6">
        <ToggleSwitch
          label={t('settings.donations.enableLeaderboard')}
          description={t('settings.donations.enableLeaderboardDesc')}
      ...
      <div className="border-t border-gray-200 pt-8">
        <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.donations.fundsAndDesignations')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('settings.donations.fundsAndDesignationsDesc')}</p>
      </div>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button type="button" variant="secondary" onClick={exportCsv}>{t('common.exportCsv')}</Button>
          <Button type="button" variant="secondary" onClick={handleImport}>{t('settings.donations.importFromCsv')}</Button>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="text-md font-semibold text-gray-900">{t('settings.donations.fundList')}</h4>
            {fundsLoading && <span className="text-xs text-gray-500">{t('common.loading')}</span>}
          </div>
          {funds.length === 0 && !fundsLoading && (
            <p className="text-sm text-gray-600">{t('settings.donations.noFunds')}</p>
          )}
        ...
        <div className="border-t border-gray-200 pt-6 space-y-4">
          <div>
            <h4 className="text-md font-semibold text-gray-900">{t('settings.donations.donationReports')}</h4>
            <p className="text-sm text-gray-500 mt-1">{t('settings.donations.donationReportsDesc')}</p>
          </div>
          ...
          {reportLoading ? (
            ...
            <div className="space-y-3">
              {donationSummaries.length === 0 ? (
                <p className="text-sm text-gray-600">{t('settings.donations.noDonationData')}</p>
              ) : (
                <>
        ...
        <div className="space-y-2">
          <h4 className="text-md font-semibold text-gray-900">{t('settings.donations.recentFundActivity')}</h4>
          {auditLogs.length === 0 ? (
            <p className="text-sm text-gray-600">{t('settings.donations.noFundChanges')}</p>
          ) : (
        ...
        <Button
          ...
          onClick={async () => {
            try {
              ...
              alert(t('settings.donations.saved'));
            } catch (error: any) {
              alert(error.message || t('settings.donations.saveFailed'));
            } finally {
              setIsSaving(false);
            }
          }}
