...
const PermissionsTab: React.FC<PermissionsTabProps> = ({ tenant, onUpdate, currentUser }) => {
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            setHasChanges(false);
            alert(t('settings.permissions.saved'));
        } catch (err: any) {
            console.error('Error saving permissions', err);
            alert(err?.message || t('settings.permissions.saveFailed'));
        }
    };
    ...
    return (
        <div className="space-y-8">
            <div>
                <h3 className="text-lg font-medium leading-6 text-gray-900">{t('settings.permissions.title')}</h3>
                <p className="mt-1 text-sm text-gray-500">{t('settings.permissions.description')}</p>
            </div>
